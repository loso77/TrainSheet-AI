...
async function recognize(){if(!file)return;E.recognizeBtn.disabled=true;E.progress.classList.remove('hidden');status(E.status,'正在本地压缩照片……');try{const image=await compress(file);status(E.status,`大模型正在识别当前配置的 ${sheetConfig.entries.length} 个表号……`);const d=await api('/recognize',{method:'POST',headers:headers(),body:JSON.stringify({image,config:apiConfig()})});rows=norm(d.rows);originalRows=rows.map(r=>({...r}));render();showQuota(d.usage);status(E.status,'识别完成：时间来自当前配置，请核对车号和股道。','success')}catch(e){if(e.status===401){state.setToken('');authUI(false)}status(E.status,e.message,'error')}finally{E.progress.classList.add('hidden');E.recognizeBtn.disabled=!file}}
