...
function render(){E.resultBody.innerHTML='';let n=0,modified=0,conflicts=0;rows.forEach((r,i)=>{const empty=!r.train_number||!r.track_name,review=Boolean(r.needs_review)||r.confidence<.88||r.ambiguity||empty;if(review)n++;if(r.train_modified||r.track_modified)modified++;if((r.review_reasons||[]).some(x=>x.includes('重复')))conflicts++;const tr=document.createElement('tr');if(review)tr.classList.add('review');if(r.ambiguity)tr.classList.add('high-risk');if(empty)tr.classList.add('empty');const reasons=(r.review_reasons||[]).join('；')||r.note||'';if(reasons)tr.title=`表号${r.table_no}：${reasons}`;tr.innerHTML=`<td class="fixed-time">${esc(r.time)}${review?'<span class="row-alert">需核对</span>':''}</td><td><input aria-label="表号${r.table_no}车号" data-i="${i}" data-k="train_number" value="${esc(r.train_number)}" placeholder="空"></td><td><input aria-label="表号${r.table_no}股道" data-i="${i}" data-k="track_name" value="${esc(r.track_name)}" placeholder="空"></td>`;E.resultBody.appendChild(tr)});E.summary.textContent=`31条记录，${n}行需要人工确认；检测到${modified}行涂改，${conflicts}行涉及重复冲突。黄色行请点按查看原因。`;E.resultCard.classList.remove('hidden');setCompareMode(true);requestAnimationFrame(()=>E.compareWorkspace.scrollIntoView({behavior:'smooth',block:'start'}))}
